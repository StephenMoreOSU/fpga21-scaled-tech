...
alignment = ""
titles = ""
for pitch in range(0, len(asap7_pitch_names)):
    alignment += "c "
    titles += "& %s " % asap7_pitch_names[pitch]
alignment = alignment[:-1] + "}\n"
titles = "     " + titles[:-1] + "\\\\\n"
template += alignment + titles
...
for pitch in range(0,len(asap7_pitch_names)):
    ...
    barrier = 1.5
    os.system("python find_optimal_ar.py --ar_max %f --pitch %d --inc_w 0.1 --t_barrier %f --eps_rel %f > ar_dump.txt"\
            % (ar_max, asap7_pitches[pitch], barrier, eps_rel))
